...
class ConvNet(nn.Module):
    def __init__(self, channel, num_classes, net_width, net_depth, net_act, net_norm, net_pooling, im_size=(32, 32)):
        ...
        self.features, shape_feat = self._make_layers(channel,
                                                      net_width,
                                                      net_depth,
                                                      net_norm,
                                                      net_act,
...
class MBConvBlock(nn.Module):
    def __init__(self, in_planes, out_planes, expand_ratio, kernel, stride, reduction, norm='batchnorm'):
        super(MBConvBlock, self).__init__()
        self.stride = stride
        self.use_residual = self.stride == 1 and in_planes == out_planes

        # Expansion phase
        expand_planes = expand_ratio * in_planes
        self.expansion = nn.Sequential()
        if expand_ratio != 1:
            self.expansion = nn.Sequential(
                nn.Conv2d(in_planes, expand_planes, kernel_size=1, bias=False),
        ...
        self.depthwise = nn.Sequential(
            nn.Conv2d(expand_planes, expand_planes, kernel_size=kernel, stride=stride, padding=kernel // 2,
                      groups=expand_planes, bias=False),
            nn.BatchNorm2d(expand_planes),
            nn.ReLU6(inplace=True)
        )
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(expand_planes, expand_planes // reduction, kernel_size=1),
            nn.ReLU6(inplace=True),
            nn.Conv2d(expand_planes // reduction, expand_planes, kernel_size=1),
            nn.Sigmoid()
        )

        # Output phase
        self.output = nn.Sequential(
            nn.Conv2d(expand_planes, out_planes, kernel_size=1, bias=False),
...
class EfficientNet(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        self.init_conv = nn.Sequential(
            nn.Conv2d(channel, 32, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU6(inplace=True)
        )
        ...
        self.blocks = nn.Sequential(
            MBConvBlock(32, 16, 1, 3, 1, 4),
            MBConvBlock(16, 24, 6, 3, 2, 4),
            MBConvBlock(24, 40, 6, 5, 2, 4),
            MBConvBlock(40, 80, 6, 3, 2, 4),
            MBConvBlock(80, 112, 6, 5, 1, 4),
            MBConvBlock(112, 192, 6, 5, 2, 4),
            MBConvBlock(192, 320, 6, 3, 2, 4)
        )
    ...
    def forward(self, x):
        ...
        out = self.bn(out)
        out = self.relu(out)
        out = F.adaptive_avg_pool2d(out, 1).reshape(out.shape[0], -1)
        out = self.fc(out)
        return out
